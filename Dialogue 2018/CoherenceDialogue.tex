...
\begin{frame}{Purpose of the Study}
  ...
  \includegraphics[width=\textwidth]{doc11358_topic0.png}\\
  \end{minipage}
  \begin{minipage}{0.3\textwidth}
      \includegraphics[width=\textwidth]{legend.png}
  \end{minipage}
  \begin{block}{Problem}
    10 most frequent words cover a small proportion of text.  Even more so, if you take into account the requirement of co-occurrence. Is analysis of co-occurrences adequate?
  \end{block}
\end{frame}
...
\begin{frame}{Topic Modeling and Matrix Factorization}
  \emph{Topic model} describes documents using latent topics.
  \begin{itemize}
    \item $\phi_{wt} \equiv p(w \mid t)$~---~ how often does word $w$ appear in topic $t$
    \item $\theta_{td} \equiv p(t \mid d)$~---~probability of topic $t$ within document $d$
  \end{itemize}
  \centering
  \includegraphics[width=\textwidth]{murat_plsa.png}\\
...
\begin{frame}{Original Dataset: three top-words of each of 19 topics}
  ...
  {\small
    $\approx 2000$ \emph{monotopic} PostNauka\footnote[frame]{https://postnauka.ru} articles. Topics are found with bigARTM\footnote[frame]{Vorontsov K. et al. Bigartm: Open source library for regularized multimodal topic modeling of large collections, 2015}
  }
...
\begin{frame}{Semisynthetic Dataset}
  ...
  \column{0.5\textwidth}
    ...
    \begin{itemize}\setlength{\itemindent}{-1em} % https://stackoverflow.com/questions/2611276/latex-beamer-way-to-change-the-bullet-indentation
      ...
      \item Then we “sew” them together in random order to produce a new document. 
